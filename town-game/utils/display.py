...
def print_separator(char="â", width=50, *color_names):
    line = char * width
...
def print_dialogue(speaker, text, mood="neutral"):
    ...
    print(color(f"ã{speaker}ã?, "bold", mood_color))
    print(f"  {color('ã?, 'dim')}{text}{color('ã?, 'dim')}")
    print()


def print_player_thought(text):
    print(color("[åå¿]", "dim", "yellow"), end=" ")
    print(color(text, "yellow"))
    print()


def print_system(text):
    print(color(f"  â?{text}", "dim"))
    print()


def print_reward(text, amount):
    print(color(f"  â?{text} +{amount}", "green", "bold"))


def print_penalty(text, amount):
    print(color(f"  â?{text} -{amount}", "red"))
...
def print_status_bar(player):
    ...
    hp_bar = "â? * hp_progress + "â? * (hp_bar_width - hp_progress)

    print_separator("â", bar_width)
    ...
    print(color(f"  éå¸: {player.gold}  |  å£°æ: {player.reputation}", "yellow"))
    ...
    print(color(f"  {fmt_attr('æè?, player.craft)}  {fmt_attr('æºæ§', player.wit)}  {fmt_attr('é­å', player.charm)}  {fmt_attr('ä½å', player.grit)}", "cyan"))

    if player.level < 15:
        print(color(f"  ç»éª: [{exp_bar}] {player.exp}/{player.get_exp_to_next()} (åçº§)", "green"))
    else:
        print(color(f"  ç»éª: [{exp_bar}] {player.exp} (MAX)", "green"))
    print_separator("â", bar_width)
    print()
...
def print_combat_header(enemy_name):
    print()
    print_separator("â?, 50, "red")
    print_centered("â?ææå¼å§?â?, 50, "bold", "red")
    print_separator("â?, 50, "red")
    print(color(f"  æäºº: {enemy_name}", "bold", "red"))
    print()


def print_enemy_status(enemy):
    hp_bar_width = 20
    hp_progress = int((enemy["生命值"] / max(1, enemy["最大生命值"])) * hp_bar_width)
    hp_bar = "â? * hp_progress + "â? * (hp_bar_width - hp_progress)
    print(color(f"  {enemy['name']}: [{hp_bar}] {enemy['hp']}/{enemy['max_hp']} HP", "red"))
    print(color(f"  æ¤ç²ç­çº§(AC): {enemy['ac']}", "yellow"))
    print()
...
def print_player_combat_status(player):
    ...
    hp_progress = int((player.hp / max(1, player.max_hp)) * hp_bar_width)
    hp_bar = "â? * hp_progress + "â? * (hp_bar_width - hp_progress)
    print(color(f"  ä½? [{hp_bar}] {player.hp}/{player.max_hp} HP", "green"))
    print(color(f"  æ»å»å å? +{player.calc_attack_bonus()}  |  AC: {player.calc_ac()}", "cyan"))
    print()
...
def print_combat_log(text, log_type="normal"):
    ...
    print(color(f"  â?{text}", color_name))


def print_combat_menu(options):
    print(color("  ââ éæ©è¡å¨ ââ", "cyan", "bold"))
